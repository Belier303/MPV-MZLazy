...
local language = {
    ["default"] = {
        ...
        download_saved = "Download saved to",
        menu = "Menu",
    },
}
...
local state = {
    ...
    flash_element = nil,                    -- element name to flash highlight (set by volume/speed observers)
    flash_until = 0,                        -- time until which the flash highlight lasts
...
local function render_elements(master_ass, osc_vis, wc_vis)
    local function render_element(n)
        ...
        -- flash highlight for external property changes (volume/speed)
        if state.flash_element ~= nil and element.name == state.flash_element then
            if mp.get_time() < state.flash_until then
                if element.styledown then
                    local flash_style = "{\\1c&H" .. osc_color_convert(user_opts.hover_effect_color) .. "&}"
                    style_ass:append(flash_style)
                end
            else
                state.flash_element = nil
            end
        end
        ...
        elseif element.type == "button" then
            ...
            if element.tooltipF ~= nil and element.enabled and not seeking_with_force_tooltip and user_opts.tooltip_hints then
                local hb = element.hover_box
                local is_flash_tooltip = state.flash_element == element.name and mp.get_time() < state.flash_until
                if (hb and mouse_hit_coords(hb.x1, hb.y1, hb.x2, hb.y2) or mouse_hit(element) or is_flash_tooltip) then
                    local tooltiplabel
...
local function download_done(success, _, error)
    if success then
        ...
        local conf_path = mp.command_native({"expand-path", "~~/ytdl_download.conf"})
        local f = io.open(conf_path, "r")
        if f then
            local c = f:read("*all")
            f:close()
            local m = c:match("download_path%s*=%s*(%S+)")
            if m then target = m end
        end
        local path = mp.command_native({"expand-path", target})
        mp.commandv("show-text", locale.download_saved .. " " .. path, "-1", "1")
    else
...
-- show subtitle track select menu with "Add Subtitle" option
local function show_sub_select()
    ...
    -- "No subtitle" option
    items[idx] = ((sid == nil or sid == "no") and "✓ " or "  ") .. "無字幕"
    idx = idx + 1

    local track_start = idx
    for _, t in ipairs(tracks) do
        if t.type == "sub" then
            local label = t.title or t.lang or ("字幕軌 " .. t.id)
            if t.external and t["external-filename"] then
                label = label .. " ［外部］"
            end
            items[idx] = ((t.selected and sid) and "✓ " or "  ") .. label
            idx = idx + 1
        end
    end
    local track_end = idx - 1
    ...
    input.select({
        ...
        items = items,
        submit = function(sel)
            if sel == add_sub_idx then
                mp.set_property_native("user-data/modernz/do-add-subtitle", true)
                return
            end
            if sel >= track_start and sel <= track_end then
                local t_idx = sel - track_start + 1
                local count = 0
                for _, t in ipairs(tracks) do
                    if t.type == "sub" then
                        count = count + 1
...
local function osc_init()
    ...
    ne.eventresponder["mbtn_left_up"] = function()
        mp.command("script-message pl-add-file")
    end
    ...
    elements["sub_track"].eventresponder["mbtn_left_up"] = function()
        local ok, err = xpcall(show_sub_select, debug.traceback)
        if not ok then mp.msg.error("sub_select: " .. tostring(err)) end
    end
    ...
    ne.tooltipF = function ()
        ...
        local spd_str = (spd % 1 == 0) and string.format("%.1f×", spd) or string.format("%g×", spd)
        return locale.speed_control .. ": " .. spd_str
    end
    bind_buttons("speed")
    ...
    local function get_dl_fmt()
        local conf_path = mp.command_native({"expand-path", "~~/ytdl_download.conf"})
        local f = io.open(conf_path, "r")
        if f then
            local c = f:read("*all")
            f:close()
            local m = c:match("output_format%s*=%s*(%S+)")
    ...
    ne.eventresponder["mbtn_left_up"] = function ()
        local localpath = get_dl_path()
        local out_fmt = get_dl_fmt()

        if state.downloading then
            mp.commandv("show-text", locale.download_in_progress, "-1", "1")
        else
            mp.commandv("show-text", locale.downloading .. " (" .. out_fmt:upper() .. ", " .. localpath .. ")", "-1", "1")
            state.downloading = true
            local command = {
                "yt-dlp",
                state.is_image and "" or get_ytdl_format(),
                "--merge-output-format", out_fmt,
                "--remux-video", out_fmt,
                "--add-metadata",
                "--embed-subs",
                "-o", "%(title)s.%(ext)s",
...
mp.observe_property("volume", "native", function(_, val)
    ...
    state.volume = val
    if state.file_loaded and user_opts.visibility ~= "never" then
        show_osc()
        reset_timeout()
        state.flash_element = "vol_ctrl"
        state.flash_until = mp.get_time() + 0.8
    end
    request_tick()
end)
...
mp.observe_property("speed", "native", function(_, val)
    if val == nil then return end
    state.speed = val
    if state.file_loaded and user_opts.visibility ~= "never" then
        show_osc()
        reset_timeout()
        state.flash_element = "speed"
        state.flash_until = mp.get_time() + 0.8
    end
    request_tick()
end)
