# 前言
程式基礎0的文科肥宅想用看看別人推薦的MPV播放器，所以用AI vibe coding改造，  
把介面改成比較喜歡的ModernZ的樣式，再微調一些我自己比較順手的使用方式，  
主要就是做出來自己用，所以不會維護，如果想要微調成自己習慣的使用操作方式，就自行拿去問AI吧!  
...
![OSD訊息與播放控制列](screenshots/OSD訊息與播放控制列.png)

## 功能特色

- **yt-dlp 下載系統**：右鍵選單或工具列一鍵下載，支援 MP4/MKV 格式切換
- **網址影片畫質選擇**：工具列畫質按鈕或右鍵選單即時切換解析度（1080p/720p/480p/僅音訊），可查詢所有可用格式
...
- **Ctrl+V 貼上網址播放**：複製網址後直接在播放器按 Ctrl+V 立即載入播放
- **內建字幕優先繁體**：多語言內建字幕時自動選取「中文（繁體）」
...
1. 下載 [mpv-lazy 原始包 mpv-lazy-20260510.exe](https://github.com/hooke007/mpv_PlayKit/releases)（內含 mpv.exe、Python、VapourSynth 等執行環境）
...
工具列畫質按鈕或右鍵選單 → 網址影片畫質，即時切換目前串流的解析度。  
預設是先在後台判斷網址影片的最高畫質，如果超過預設的1080P就會維持1080P  
例如：  
網址影片最高畫質為4K，播放器就會顯示1080P  
網址影片最高畫質為1080P，播放器就會顯示1080P  
網址影片最高畫質為720P，播放器就會顯示720P  

| 選項 | 對應 ytdl-format |
|------|-----------------|
| 自動（上限 1080p） | `bestvideo[height<=?1080]+bestaudio/best` |
| 最佳畫質（無上限） | `bestvideo+bestaudio/best` |
| 1080p | `bestvideo[height<=?1080]+bestaudio/best` |
| 720p | `bestvideo[height<=?720]+bestaudio/best` |
| 480p | `bestvideo[height<=?480]+bestaudio/best` |
| 僅音訊 | `bestaudio/best` |

按「檢視所有可用格式」可查詢 yt-dlp 回傳的完整格式清單，直接選取特定格式 ID。
...
| 播放清單 | 播放清單 | 總選單 |
| 截圖 | 截圖（原始） | 開啟截圖資料夾 |
| 畫質 | 畫質選擇選單（input.select） | 畫質選擇選單 |
| 全螢幕 | 切換 | 最大化 |
| 速度 | 加速 0.1 | 重置 1x |
| 視窗頂置 | 切換 | — |

> 截圖按鈕：左鍵截圖、右鍵開啟資料夾、中鍵（滾輪鍵）開啟截圖設定選單
...
- 新增 yt-dlp 下載系統（右鍵選單 + 工具列按鈕，支援 MP4/MKV 格式切換）
...
- 工具列新增畫質按鈕，點擊開啟 `input.select` 解析度選單（支援 1080p/720p/480p/僅音訊／檢視所有可用格式）
- 右鍵選單新增網址影片畫質區段（`menu.conf`）
- 新增 Ctrl+V 貼上網址播放功能（`modernz.lua`）
- 新增 auto_trad_sub.lua 自動優先選取繁體中文內建字幕
